...
tidyT$gene <- factor(tidyT$gene, levels = genes)
...
p <- ggplot(data = tidyT, aes(x = gene, y = expression)) +
      ...
      theme(axis.text.x=element_text(size=9, margin = margin(r = 600))) +
      ...
      theme(axis.text.x=element_text(angle=45,hjust=1))
  		#coord_flip()
ggsave(paste0("plots/Fig1I2_PreAnnotation_TCellViolin_", Sys.Date(), ".pdf"),
       p, height = 2.5, width = 4, bg = "transparent")
